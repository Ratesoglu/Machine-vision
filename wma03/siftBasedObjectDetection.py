...
def finderSIFT(sample, video):
    ...
    result = cv2.VideoWriter('wma03/resultSIFT.avi',  cv2.VideoWriter_fourcc(*'MJPG'), 20, size)
...
def finderORB(sample, video):
    ...
    frame_height = int(video.get(4))
    size = (frame_width, frame_height)
    result = cv2.VideoWriter('wma03/resultORB.avi',  cv2.VideoWriter_fourcc(*'MJPG'), 20, size)
...
def main():
    sample = cv2.imread('wma03/WMRpila/saw1.jpg')
    sample = resize(sample, 4)   
    cap = cv2.VideoCapture('wma03/WMRpila/sawmovie.mp4')
    #finderSIFT(sample, cap)
    finderORB(sample, cap)
